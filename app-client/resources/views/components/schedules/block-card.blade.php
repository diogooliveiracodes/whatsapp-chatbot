...
@php
    ...
    if ($block->block_type->value === 'full_day') {
        ...
        $blockDateTime = \Carbon\Carbon::parse($block->block_date, $userTimezone)->setTimeFromTimeString($block->end_time);
    }
    ...
    <div class="p-2">
        ...
        <div class="flex items-center justify-between mb-1">
            <h3 class="text-sm font-semibold text-red-900 dark:text-red-200 truncate">
                ...
                @else
                    {{ \Carbon\Carbon::parse($block->start_time, $userTimezone)->format('H:i') }} - {{ \Carbon\Carbon::parse($block->end_time, $userTimezone)->format('H:i') }}
                @endif
            </h3>
        </div>
        @if($block->reason)
